...
#ifndef STLCACHE_POLICY_LFUSTAR_MULTI_HPP_INCLUDED
#define STLCACHE_POLICY_LFUSTAR_MULTI_HPP_INCLUDED
...
namespace stlcache {
    template <class Key,template <typename T> class Allocator> class _policy_lfustar_multi_type : public virtual _policy_lfu_type<Key,Allocator> {
        typedef set<Key,less<Key>,Allocator<Key> > keySet;
    public:
        _policy_lfustar_multi_type(const size_t& size ) throw() : _policy_lfu_type<Key,Allocator>(size) { }
        _policy_lfustar_multi_type(const _policy_lfustar_multi_type& x ) throw() : _policy_lfu_type<Key,Allocator>(x) { }
     ...
     */
    struct policy_lfustar_multi {
        template <typename Key, template <typename T> class Allocator>
            struct bind : _policy_lfustar_multi_type<Key,Allocator> { 
                bind(const bind& x) : _policy_lfu_type<Key,Allocator>(x), _policy_lfustar_multi_type<Key,Allocator>(x)  { }
                bind(const size_t& size) : _policy_lfu_type<Key,Allocator>(size), _policy_lfustar_multi_type<Key,Allocator>(size) { }
            };
    };
}
