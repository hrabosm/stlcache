...
#ifndef STLCACHE_POLICY_LFUAGINGSTAR_MULTI_HPP_INCLUDED
#define STLCACHE_POLICY_LFUAGINGSTAR_MULTI_HPP_INCLUDED
...
namespace stlcache {
    template <time_t Age,class Key,template <typename T> class Allocator> class _policy_lfuagingstar_multi_type : public virtual _policy_lfuaging_type<Age,Key,Allocator>, public virtual _policy_lfustar_type<Key,Allocator> {
    public:
        _policy_lfuagingstar_multi_type(const size_t& size ) throw() : _policy_lfuaging_type<Age,Key,Allocator>(size), _policy_lfustar_type<Key,Allocator>(size),_policy_lfu_type<Key,Allocator>(size) { }
     ...
     * \see policu_lfustar
     */
    template <time_t Age> struct policy_lfuagingstar_multi {
        template <typename Key, template <typename T> class Allocator>
            struct bind : _policy_lfuagingstar_multi_type<Age,Key,Allocator> { 
                bind(const bind& x) : _policy_lfu_type<Key,Allocator>(x), _policy_lfuaging_type<Age,Key,Allocator>(x),_policy_lfustar_type<Key,Allocator>(x),_policy_lfuagingstar_multi_type<Age,Key,Allocator>(x)  { }
                bind(const size_t& size) : _policy_lfu_type<Key,Allocator>(size), _policy_lfuaging_type<Age,Key,Allocator>(size),_policy_lfustar_type<Key,Allocator>(size),_policy_lfuagingstar_multi_type<Age,Key,Allocator>(size)  { }
            };
    };
}
