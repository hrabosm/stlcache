...
#ifndef STLCACHE_POLICY_LFUAGING_MULTI_HPP_INCLUDED
#define STLCACHE_POLICY_LFUAGING_MULTI_HPP_INCLUDED
...
namespace stlcache {
    template <time_t Age,class Key,template <typename T> class Allocator> class _policy_lfuaging_multi_type : public virtual _policy_lfu_type<Key,Allocator> {
        typedef set<Key, less<Key>, Allocator<Key> > keySet;
        map<Key,time_t, less<Key>, Allocator<pair<const Key, time_t> > > _timeKeeper;
        time_t _oldestEntry;
        time_t age;
    public:
        _policy_lfuaging_multi_type<Age,Key,Allocator>& operator= ( const _policy_lfuaging_multi_type<Age,Key,Allocator>& x) throw() {
        ...
        _policy_lfuaging_multi_type(const _policy_lfuaging_multi_type<Age,Key,Allocator>& x)  throw() : _policy_lfu_type<Key,Allocator>(x) {
            *this=x;
        }
        _policy_lfuaging_multi_type(const size_t& size ) throw() : _policy_lfu_type<Key,Allocator>(size) { 
            this->age=Age;
        ...
        virtual void swap(policy<Key,Allocator>& _p) throw(exception_invalid_policy) {
            try {
                _policy_lfuaging_multi_type<Age,Key,Allocator>& _pn=dynamic_cast<_policy_lfuaging_multi_type<Age,Key,Allocator>& >(_p);
     ...
     */
    template <time_t Age> struct policy_lfuaging_multi {
        template <typename Key, template <typename T> class Allocator>
            struct bind : _policy_lfuaging_multi_type<Age,Key,Allocator> { 
                bind(const bind& x) : _policy_lfu_type<Key,Allocator>(x), _policy_lfuaging_multi_type<Age,Key,Allocator>(x)  { }
                bind(const size_t& size) : _policy_lfu_type<Key,Allocator>(size), _policy_lfuaging_multi_type<Age,Key,Allocator>(size) { }
            };
    };
}
